...
define([],
  function () {
    ...
    const announce = function (message: string, manner?: string): void {
      if (manner === undefined || !validAriaLiveValues.includes(manner)) {
        manner = 'polite';
      }

      const params: CustomEventInit = {
        'bubbles': true,
        'detail': { 'message': message, 'manner': manner }
      };
      
      const element = document.getElementById('globalBody');
      if (element) {
        element.dispatchEvent(new CustomEvent('announce', params));
      }
    };
